...
class PCIe_FootprintWizard(HFPW.HelpfulFootprintWizardPlugin):

    def GetName(self):
        return "PCIe (SMT connector)"

    def GetDescription(self):
        return "PCIe (SMT connector) Footprint Wizard"
    ...
    def GenerateParameterList(self):
        self.AddParam( "Pads", "n", self.uNatural, 36 )
        self.AddParam( "Pads", "pitch", self.uMM, 1 )
        self.AddParam( "Pads", "width", self.uMM, 0.7 )
        self.AddParam( "Pads", "height", self.uMM, 3 )
        self.AddParam( "Pads", "deep", self.uMM, 5.6 )
        self.AddParam( "Comb", "width", self.uMM, 3 )
    ...
    def BuildThisFootprint(self):
        ...
        pad_deep        = p["Pads"]["deep"]
        cmb_width       = p["Comb"]["width"]
        shl_width       = p["Shield"]["width"]
        shl_height      = p["Shield"]["height"]
        shl_to_pad      = p["Shield"]["shield_to_pad"]
        shl_from_top    = p["Shield"]["from_top"]

        offsetX         = (cmb_width+ pad_pitch * ( pad_count-1 )) / 2
        ...
        pad1count = pad_count
        pad2count = pad_count-11
        
        if pad1count>11:
            pad1count =11
        if pad2count<0:
            pad2count =0
        for n in range ( 0, pad1count ):
            xpos = pad_pitch*n - offsetX
            pad = self.smdRectPad(self.module,size_pad, pcbnew.wxPoint(xpos,0),str(n+1))
            self.module.Add(pad)

        offsetX2=xpos + pad_pitch + cmb_width
        for n in range ( 0, pad2count ):
            xpos = pad_pitch*n + offsetX2
            pad = self.smdRectPad(self.module,size_pad, pcbnew.wxPoint(xpos,0),str(n+12))
            self.module.Add(pad)
        ...
        xpos = (cmb_width+(pad_count-1) * pad_pitch)+shl_to_pad - offsetX
        ...
        xend = (cmb_width+pad_pitch * pad_count )+ xstart;
        ...
        yend = pad_s0_pos.y + shl_height/2 + margin + pad_deep
